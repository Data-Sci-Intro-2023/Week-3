...
This lesson has six exercises that need to be completed.
...
In hydrology it is common to area-normalize stream flow data. That is, we divide our cubic discharge measurements by the location's drainage area to get units in length/time. Using `dataRetrieval::readNWISsite`, area-normalize the discharge data. Remake  and make the same comparison with the normalized values.

```{r}
meta <- dataRetrieval::readNWISsite(siteNumbers = c("402114105350101","06741510","06746095","06752260")) %>%
  select(site_no, drain_area_va)

new_q <- data %>%
  left_join(meta,by="site_no") %>%
  mutate(mm_day=1000*(q_cfs*0.028316847000000252*86400)/(drain_area_va*1000000))
 
  
plotly::ggplotly(ggplot(data = new_q) +
  geom_line(aes(x=Date, y=mm_day, color = site_no)) +
  theme_bw() +
  facet_wrap(~watershed, ncol=1)
  )
```
